...
export const featuredProjectCaseStudies: ProjectCaseStudy[] = [
  ...
  {
    eyebrow: "Verified AI Prototype",
    title: "Prosuite Chatbot Hackathon",
    summary:
      "A Next.js 16 GRC dashboard prototype with Mazwi, a module-aware assistant backed by a server-side OpenAI streaming route and seeded enterprise demo data.",
    links: [
      {
        label: "View GitHub Repo",
    ...
    screenshots: [
      {
        src: "/projects/prosuite/dashboard.jpg",
        alt: "Prosuite GRC dashboard with risk metrics, heatmap, recent activity, and module navigation",
        label: "GRC Operations Dashboard",
        detail:
          "Seeded risk, asset, incident, compliance, audit, and governance signals organized into one operational view.",
      },
      {
        src: "/projects/prosuite/mazwi-chat.jpg",
        alt: "Mazwi GRC assistant open over the Prosuite dashboard with risk suggestions",
        label: "Module-Aware Mazwi Assistant",
        detail:
          "The embedded assistant changes its suggested prompts with the active GRC module and streams responses from a server-side route.",
      },
    ],
    role: [
      "Built the assistant experience inside a multi-module GRC dashboard rather than as an isolated chatbot page.",
      "Implemented a server-side OpenAI chat route with `gpt-4o-mini` as the default model and streamed responses using server-sent events.",
      "Hardened the prototype with typed action/context helpers, controlled missing-key behavior, a clean production build, clean lint, and mobile overflow verification.",
    ],
    stack: [
      "Next.js 16",
      "React 19",
    ...
    features: [
      "GRC dashboards and workflows for risk, asset, incident, audit, compliance, governance, and performance modules.",
      "Mazwi assistant with route-aware module context, quick suggestions, chat history, preferences, voice-input UI, and Markdown responses.",
      "Streaming `/api/chat` route with seeded business context and response-driven heatmap, chart, metric, progress, and table triggers.",
      "Approval-aware action concepts for create, update, delete, escalation, assignment, reporting, search, and evidence harvesting.",
    ],
    decisions: [
      "Kept the OpenAI key server-side and return a controlled `503` when AI configuration is absent.",
      "Used deterministic seeded JSON data so the public prototype can demonstrate rich GRC workflows without exposing client data.",
      "Required approval for destructive AI action concepts and kept those helpers separate from the streaming response route.",
      "Positioned this honestly as a prototype because persistence, tenant isolation, sensitive-data masking, and long-term retrieval are not production-ready.",
    ],
    proofLabel: "Verified Prototype Proof",
    proofNeeded: [
      "The public README documents OpenAI, `gpt-4o-mini`, setup, demo prompts, architecture, screenshots, and known boundaries.",
      "`npm run lint` passes with zero errors and warnings; the Next.js production build compiles, type-checks, and generates all 57 routes.",
      "`npm audit --omit=dev` reports zero production dependency vulnerabilities.",
      "The Browser verified desktop navigation, the Mazwi panel, module-aware suggestions, the mobile drawer, and a 390px page with no horizontal overflow.",
      "A live assistant-response capture remains pending because it requires an approved OpenAI key and a deliberate paid demo prompt.",
    ],
  },
  {
